...
## Session: Mail Port Phases 2–4

Extended the JMAP mail port from 6 read-only commands to 17 full commands, plus a Kirigami mail client.

**Phase 2 — Send & Compose (3 commands):**
- `identities` — list sender identities via JMAP Identity/get
- `send` — compose RFC 5322 message, import via JMAP, submit for SMTP delivery
- `reply` — fetch original headers (Message-ID, References), build threaded reply, submit

**Phase 3 — Mail Management (8 commands):**
- `move` / `delete` — mailbox reassignment or permanent destroy
- `flag` / `unflag` / `mark_unread` — keyword manipulation ($flagged, $seen)
- `search` — full-text query across all mailboxes
- `attachment_list` / `attachment_download` — list metadata, download by blob ID to `/tmp/appmesh-mail/`

**Phase 4 — Kirigami Mail Client:**
- `qml/mail/main.cpp` — QApplication entry point with Widgets for Kirigami
- `qml/mail/Main.qml` — three-pane Kirigami UI (~670 lines): sidebar with mailbox list + search, message list with from/subject/date/preview, detail page with headers + body + actions (reply, flag, move, delete, mark unread)
- Compose and Reply dialogs with quoted text and reply-position toggle (above/below quote)
- Move dialog lists all mailboxes
- Login dialog with JMAP credentials
- CMakeLists.txt updated: `KF6Kirigami` dependency, `appmesh-mail` target

**Key implementation details:**
- `build_rfc5322()` — minimal RFC 5322 message builder (From, To, Subject, Date, Message-ID, MIME-Version, Content-Type, optional In-Reply-To/References)
- `rfc2822_date()` / `days_to_ymd()` — pure Rust date formatting, no chrono dependency
- `find_first_identity()` — stateless identity lookup for send/reply From header
- All new commands follow existing pattern: `require_client()` → JMAP call → `PortValue` result

**Files:** 2 new (`qml/mail/main.cpp`, `qml/mail/Main.qml`), 3 modified (`mail.rs` +737/-51, `ports.php` +41, `CMakeLists.txt` +16). Both `cargo build --release` and CMake build clean.
