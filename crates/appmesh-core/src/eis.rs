...
impl EisConnection {
    ...
    pub fn connect(
        ...
        if verbose {
            eprintln!("appmesh: context created, starting handshake");
        }
        ...
        if verbose {
            eprintln!("appmesh: handshake complete, serial={}", last_serial);
            eprintln!("appmesh: negotiated interfaces: {:?}", resp.negotiated_interfaces);
        }
        ...
        while let Some(result) = context.pending_event() {
            ...
            {
                if verbose {
                    eprintln!("appmesh: responding to post-handshake ping");
                }
                ping.done(0);
            }
        }
        ...
        if verbose {
            eprintln!("appmesh: sent sync + ping response");
        }
        ...
        while !ready && timeout_count < max_timeouts {
            ...
            while let Some(result) = context.pending_event() {
                ...
                let request = match result {
                    ...
                    PendingRequestResult::InvalidObject(id) => {
                        if verbose {
                            eprintln!("appmesh: invalid object {}", id);
                        }
                        continue;
                    }
                };
                ...
                match request {
                    ei::Event::Connection(_connection, req) => match req {
                        ei::connection::Event::Seat { seat: _ } => {
                            if verbose {
                                eprintln!("appmesh: seat announced");
                            }
                        }
                        ei::connection::Event::Ping { ping } => {
                            if verbose {
                                eprintln!("appmesh: responding to ping");
                            }
                            ping.done(0);
                            context.flush()?;
                        }
                        ei::connection::Event::Disconnected { last_serial, reason, explanation } => {
                            eprintln!("appmesh: DISCONNECTED! serial={}, reason={:?}, explanation={:?}",
                    ...
                    ei::Event::Seat(seat, req) => match req {
                        ei::seat::Event::Capability { mask, interface } => {
                            if verbose {
                                eprintln!("appmesh: seat capability: {} mask={}", interface, mask);
                            }
                        ...
                        ei::seat::Event::Done => {
                            ...
                            if verbose {
                                eprintln!("appmesh: binding all capabilities, mask={}", combined);
                            }
                            seat.bind(combined);
                            context.flush()?;
                        }
                        ei::seat::Event::Device { device: _ } => {
                            ...
                            if verbose {
                                eprintln!("appmesh: device announced");
                            }
                        }
                        _ => {}
                    },
                    ei::Event::Device(device, req) => match req {
                        ei::device::Event::Interface { object } => {
                            if verbose {
                                eprintln!("appmesh: device interface: {}", object.interface());
                            }
                            device_interfaces
                        ...
                        ei::device::Event::Done => {
                            if let Some(obj) = device_interfaces.get("ei_keyboard") {
                                if let Some(kb) = obj.clone().downcast::<ei::Keyboard>() {
                                    if verbose {
                                        eprintln!("appmesh: keyboard device found");
                                    }
                                    keyboard = Some(kb);
                        ...
                        ei::device::Event::Resumed { serial } => {
                            ...
                            if keyboard.is_some() {
                                if verbose {
                                    eprintln!("appmesh: device resumed, serial={}", serial);
                                }
                                ready = true;
                            }
                        }
                    ...
                    ei::Event::Keyboard(_kb, ref _evt) => {
                        if verbose {
                            eprintln!("appmesh: keyboard event (keymap etc.)");
                        }
                    }
                    _ => {}
                }
            ...
            match poll_readable(&context) {
                ...
                Ok(false) => {
                    timeout_count += 1;
                    if verbose {
                        eprintln!("appmesh: poll timeout {}/{}", timeout_count, max_timeouts);
                    }
                }
        ...
        if verbose {
            eprintln!("appmesh: ready to type");
        }
    ...
    pub fn type_text(
        ...
        for c in text.chars() {
            if let Some(ki) = keymap::char_to_key(c) {
                ...
                eprintln!("appmesh: skipping unmapped char '{}'", c.escape_debug());
            }
        }
        Ok(())
    }
